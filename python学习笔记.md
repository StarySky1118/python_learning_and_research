...
Key 值不能为字典，Value 不受限。

字典的嵌套：

![image-20221018195327269](img/image-20221018195327269.png)

```python
# 字典的嵌套
grade_dict = {"王力宏": {"语文": 77, "数学": 66, "英语": 33}}
grade = grade_dict["王力宏"]["语文"]
print(f"王力宏的语文成绩为{grade}")
```
...
`list(容器)`、`tuple(容器)`、`str(容器)` 和 `set(容器)`。

### 3、容器通用排序功能

`sorted(容器)`

并不会改变原有容器，只会得到一个新的列表对象。默认升序。

如果需要降序：`sorted(容器,reverse=True)`
